...
Интеграция `FDOnAir` с `Bitfocus Companion` через bridge.
...
1. Проверьте, что в `release\server\` лежат:
   - `onAir2Companion.exe`
   - `config.json`
2. Убедитесь, что в системе установлен программный пакет Forward (SLMessageServer),
...
Часть материалов проекта (документация и отдельные технические правки) была подготовлена с помощью ИИ и затем проверена вручную.
