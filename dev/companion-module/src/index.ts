...
type TemplateCategory = 'Player' | 'Shedule' | 'Folder' | 'Setting' | 'Loading' | 'MirrorSave'
...
class OnAirBridgeInstance extends InstanceBase<ModuleConfig> {
	private config: ModuleConfig = {}
	private tcp: TCPHelper | undefined
	private lineBuffer = ''
	private requestId = 1

	private bridgeState: BridgeState = 'disconnected'
	private lastMessage = ''
	private lastBody = ''
	private lastSubject = ''
	private lastFromMachine = ''
	private lastFromQueue = ''
		...
		return {
			...
			message_match: {
				...
				callback: (feedback) => {
					...
					if (subjectFilter && this.lastSubject !== subjectFilter) return false
					if (queueFilter && this.lastFromQueue !== queueFilter) return false
	...
	private makeCommandPreset(buttonText: string, name: string, command: string, activeColor: number): CompanionPresetDefinitions[string] {
		return {
			type: 'button',
	...
	private makeTemplatePresets(): CompanionPresetDefinitions {
		const out: CompanionPresetDefinitions = {}
	...
	private connectTcp(): void {
		...
		this.tcp.on('status_change', (status, message) => {
			if (status === InstanceStatus.Connecting) this.bridgeState = 'connecting'
			if (status === InstanceStatus.Disconnected) this.bridgeState = 'disconnected'
			if (status === InstanceStatus.Ok) this.bridgeState = 'connected'
			if (status === InstanceStatus.UnknownError) this.bridgeState = 'error'
	...
	private consumeData(chunk: string): void {
	...
	private handleLine(line: string): void {
		...
		if (parsed.body !== undefined) this.lastBody = String(parsed.body)
		if (parsed.subject !== undefined) this.lastSubject = String(parsed.subject)
		if (parsed.from_machine !== undefined) this.lastFromMachine = String(parsed.from_machine)
		if (parsed.from_queue !== undefined) this.lastFromQueue = String(parsed.from_queue)
		if (parsed.type === 'response') this.lastResponse = line
		if (parsed.error) this.lastError = String(parsed.error)
		if (parsed.type === 'onair_message') {
			this.log(
				'info',
				`onair_message from=${String(parsed.from_machine ?? '')}/${String(parsed.from_queue ?? '')} subject=${String(parsed.subject ?? '')} body=${String(parsed.body ?? '')}`,
			)
		}
	...
	private matchBody(mode: string, pattern: string): boolean {
		if (mode === 'equals') return this.lastBody === pattern
		if (mode === 'regex') {
			try {
	...
	private composeTemplateCommand(template: CommandTemplate, argsOverride: string): string {
		const args = argsOverride.trim() || template.args
		if (!args || args === '?') return template.command
